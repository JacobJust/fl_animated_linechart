...
class _GestureWrapper extends StatefulWidget {
  ...
  const _GestureWrapper(this._height, this._width, this._chart, this._animation, {Key key,}) : super(key: key);
...
class _GestureWrapperState extends State<_GestureWrapper> {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      child: _AnimatedChart(widget._chart, widget._width, widget._height, _horizontalDragActive, _horizontalDragPosition, animation: widget._animation,),
      onTapDown: (tap) {
...
class _AnimatedChart extends AnimatedWidget {
  ...
  _AnimatedChart(this._chart, this._width, this._height, this._horizontalDragActive, this._horizontalDragPosition, {Key key, Animation animation}) : super(key: key, listenable: animation);
...
class ChartPainter extends CustomPainter {
  ...
  void _drawHighlights(Size size, Canvas canvas) {
    ...
    if (_horizontalDragPosition > LineChart.axisOffsetPX && _horizontalDragPosition < size.width) {
      canvas.drawLine(Offset(_horizontalDragPosition, 0), Offset(_horizontalDragPosition, size.height - LineChart.axisOffsetPX), _linePainter);
    }
    ...
    highlights.forEach((highlight) {
      ...
      TextSpan span = new TextSpan(style: new TextStyle(color: _chart.lines[index].color, fontWeight: FontWeight.w200, fontSize: 12), text: '$prefix: ${highlight.yValue.toStringAsFixed(1)} ${_chart.lines[index].unit}');
  ...
  void _drawAxisValues(Canvas canvas, Size size) {
    ...
    for (int c = 0; c <= (_stepCount + 1); c++) {
      ...
      tp.paint(canvas, new Offset(_chart.axisOffSetWithPadding - tp.width, (size.height - 6)- (c * _chart.heightStepSize) - LineChart.axisOffsetPX));
    }

    if (_chart.yAxisCount == 2) {
      for (int c = 0; c <= (_stepCount + 1); c++) {
        TextPainter tp = _chart.yAxisTexts(1)[c];
        tp.paint(canvas, new Offset(LineChart.axisMargin + size.width - _chart.xAxisOffsetPXright, (size.height - 6)- (c * _chart.heightStepSize) - LineChart.axisOffsetPX));
      }
    }

    //TODO: calculate and cache
    for (int c = 0; c <= (_stepCount + 1); c++) {
      _drawRotatedText(canvas, _chart.xAxisTexts[c], _chart.axisOffSetWithPadding + (c * _chart.widthStepSize), size.height - (LineChart.axisOffsetPX-5), pi * 1.5);
    }
  }
  ...
  void _drawUnits(Canvas canvas, Size size) {
      if (_chart.indexToUnit.length > 0) {
        ...
        TextSpan span = new TextSpan(style: new TextStyle(color: color, fontWeight: FontWeight.w200, fontSize: 14), text: _chart.indexToUnit[0]);
        TextPainter tp = new TextPainter(text: span, textAlign: TextAlign.right, textDirection: TextDirectionHelper.getDirection());
        tp.layout();
      ...
      if (_chart.indexToUnit.length == 2) {
        ...
        TextSpan span = new TextSpan(style: new TextStyle(color: color, fontWeight: FontWeight.w200, fontSize: 14), text: _chart.indexToUnit[1]);
        TextPainter tp = new TextPainter(text: span, textAlign: TextAlign.right, textDirection: TextDirectionHelper.getDirection());
        tp.layout();

        tp.paint(canvas, new Offset(size.width - tp.width - _chart.xAxisOffsetPXright, -16));
      }
  }

  void _drawGrid(Canvas canvas, Size size) {
    canvas.drawRect(Rect.fromLTWH(_chart.xAxisOffsetPX, 0, size.width - _chart.xAxisOffsetPX - _chart.xAxisOffsetPXright, size.height - LineChart.axisOffsetPX), _gridPainter);
    
    for(double c = 1; c <= _stepCount; c ++) {
      canvas.drawLine(Offset(_chart.xAxisOffsetPX, c*_chart.heightStepSize), Offset(size.width - _chart.xAxisOffsetPXright, c*_chart.heightStepSize), _gridPainter);
      canvas.drawLine(Offset(c*_chart.widthStepSize + _chart.xAxisOffsetPX, 0), Offset(c*_chart.widthStepSize + _chart.xAxisOffsetPX, size.height-LineChart.axisOffsetPX), _gridPainter);
    }
  }
