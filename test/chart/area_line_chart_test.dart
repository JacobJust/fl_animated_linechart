...
void main() {
  test('path calculate', () async {
    ...
    AreaLineChart lineChart = AreaLineChart([ChartLineHelper.createLine(10, 2, Colors.cyan, 'W')], Dates(DateTime.now(), DateTime.now()));
    lineChart.initialize(200, chartHeight);

    expectPathCacheToMatch(lineChart.getAreaPathCache(0), 10, 2, lineChart, chartHeight, 'W', lineChart.xAxisOffsetPX, lineChart.getPathCache(0).getBounds());
  });
...
void expectPathCacheToMatch(Path pathCache, int pointCount, double pointFactor, LineChart lineChart, double chartHeight, String unit, double xAxisOffsetPX, Rect bounds) {
